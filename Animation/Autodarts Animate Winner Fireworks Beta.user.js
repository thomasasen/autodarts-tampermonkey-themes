// ==UserScript==
// @name         Autodarts Animate Winner Fireworks [Beta]
// @namespace    https://github.com/thomasasen/autodarts-tampermonkey-themes
// @version      3.8-beta.1
// @description  Zeigt nach einem Sieg einen frei wählbaren Feuerwerks- und Konfetti-Effekt.
// @xconfig-description  Beta: Optionaler neonburst-Style via mo.js mit GameState-Diffing und Fail-soft-Fallback.
// @xconfig-title  Sieger-Feuerwerk [Beta]
// @xconfig-variant      all
...
// @require      https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/vendor/canvas-confetti.browser.js
// @require      https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/vendor/mo.umd.min.js
// @grant        none
// @downloadURL  https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/Autodarts%20Animate%20Winner%20Fireworks%20Beta.user.js
// @updateURL    https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/Autodarts%20Animate%20Winner%20Fireworks%20Beta.user.js
// ==/UserScript==

(function () {
  "use strict";

  const INSTANCE_KEY = "__adExtWinnerFireworksBeta";
...
`;
  ...
  let periodicCheckHandle = 0;
  let cleanedUp = false;
  let lastWinnerToken = "";
  const activeMoBursts = new Set();
  ...
  function emitNeonBurstAt(x, y, colors) {
    ...
    try {
      ...
      activeMoBursts.add(burst);
      burst.replay();
      scheduleTimeout(() => {
        activeMoBursts.delete(burst);
      }, 850);
      return true;
    } catch (_) {
      return false;
  ...
  function startNeonBurstPreset() {
    const tuning = STYLE_TUNING.neonburst;
    const colors = getThemeColors("special");
    const emit = () => {
      const width = window.innerWidth || 1280;
      const height = window.innerHeight || 720;
      const centerX = width * randomInRange(0.42, 0.58);
      const centerY = height * randomInRange(0.25, 0.52);
      const sideOffset = width * randomInRange(0.14, 0.24);
      const ringCount = Math.max(1, Number(tuning.rings) || 3);

      for (let i = 0; i < ringCount; i += 1) {
        const yJitter = height * randomInRange(-0.07, 0.07);
        scheduleTimeout(() => {
          emitNeonBurstAt(centerX, centerY + yJitter, colors);
          emitNeonBurstAt(Math.max(0, centerX - sideOffset), centerY + yJitter * 0.8, colors);
          emitNeonBurstAt(Math.min(width, centerX + sideOffset), centerY + yJitter * 0.8, colors);
        }, i * 90);
      }
    };
  ...
  function getEffectiveStyle() {
    if (CONFIG.style === "neonburst" && (!mojsLib || typeof mojsLib.Burst !== "function")) {
      return "realistic";
    }
    return CONFIG.style;
  }
  ...
  const STYLE_RUNNERS = Object.freeze({
    ...
    neonburst: startNeonBurstPreset,
  });

  function startStyle() {
    const runner = STYLE_RUNNERS[getEffectiveStyle()] || STYLE_RUNNERS.realistic;
    runner();
  }
  ...
  function getWinnerTokenFromGameState() {
    ...
    const winnerIndex = Number.isFinite(matchWinner) && matchWinner >= 0
      ? matchWinner
  ...
  function hasWinnerFromGameState() {
    return Boolean(getWinnerTokenFromGameState());
  }
  ...
  function checkWinner(source = "check") {
    ...
    const winnerToken = getWinnerTokenFromGameState();
    const stateWinnerVisible = Boolean(winnerToken);
    const winnerTokenChanged = Boolean(winnerToken) && winnerToken !== lastWinnerToken;
    ...
    const signalKey = `${domWinnerVisible}|${stateWinnerVisible}|${winnerTokenChanged}|${variantAllowed}|${variantText}`;
    ...
    if ((activeWinnerVisible && !lastWinnerVisible) || winnerTokenChanged) {
      dismissedForCurrentWin = false;
      showEffect(source);
    ...
    if (winnerToken) {
      lastWinnerToken = winnerToken;
    } else if (!activeWinnerVisible) {
      lastWinnerToken = "";
    }
  ...
  function cleanupInstance() {
    ...
    window.removeEventListener(XCONFIG_ACTION_EVENT_NAME, onXConfigSettingAction);
    ...
    if (periodicCheckHandle) {
      clearInterval(periodicCheckHandle);
      periodicCheckHandle = 0;
    }

    hideEffect("cleanup");

    if (window[INSTANCE_KEY] && window[INSTANCE_KEY].cleanup === cleanupInstance) {
  ...
  if (gameStateShared && typeof gameStateShared.subscribe === "function") {
    unsubscribeState = gameStateShared.subscribe(onGameStateUpdate);
  } else {
    periodicCheckHandle = setInterval(scheduleCheck, 900);
  }
  ...
  debugLog("init", {
    style: CONFIG.style,
    effectiveStyle: getEffectiveStyle(),
    farbe: CONFIG.colorTheme,
    intensitaet: CONFIG.intensity,
    includeBullOut: CONFIG.includeBullOut,
    ...
    hasMojs: Boolean(mojsLib && typeof mojsLib.Burst === "function"),
