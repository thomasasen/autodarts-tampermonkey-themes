// ==UserScript==
// @name         Autodarts Animate Checkout Board Targets [Beta]
// @namespace    https://github.com/thomasasen/autodarts-tampermonkey-themes
// @version      2.3-beta.1
// @description  Markiert in X01 mögliche Checkout-Ziele direkt auf dem virtuellen Dartboard.
// @xconfig-description  Beta: Zusätzlicher Bull/Double-Ripple (mo.js) bei Zielwechsel, fail-soft integriert.
// @xconfig-title  Checkout-Ziele am Board [Beta]
// @xconfig-variant      x01
...
// @grant        none
// @downloadURL  https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/Autodarts%20Animate%20Checkout%20Board%20Targets%20Beta.user.js
// @updateURL    https://raw.githubusercontent.com/thomasasen/autodarts-tampermonkey-themes/main/Animation/Autodarts%20Animate%20Checkout%20Board%20Targets%20Beta.user.js
// ==/UserScript==

(function () {
	"use strict";

	const INSTANCE_KEY = "__adExtCheckoutBoardTargetsBeta";
...
`;
   ...
   */
	...
	function setManagedTimeout(callback, delayMs) {
		const handle = setTimeout(() => {
			managedTimeouts.delete(handle);
			callback();
		}, Math.max(0, Number(delayMs) || 0));
		managedTimeouts.add(handle);
		return handle;
	}

	function clearManagedTimeouts() {
		managedTimeouts.forEach((handle) => clearTimeout(handle));
		managedTimeouts.clear();
	}

	function clearMoObjects() {
		activeMoObjects.forEach((entry) => {
			try {
				if (entry && typeof entry.tune === "function") {
					entry.tune({ isRunLess: true });
				}
			} catch (_) {
				// ignore
			}
		});
		activeMoObjects.clear();
	}

	function toTargetToken(target) {
		if (!target || typeof target !== "object") {
			return "";
		}
		const value = Number.isFinite(target.value) ? target.value : "";
		return `${target.ring || ""}:${value}`;
	}

	function getTargetCenterPoint(radius, target) {
		const ratios = CONFIG.ringRatios;
		if (!target) {
			return null;
		}
		...
		if (target.ring === "SB") {
			const r = radius * ((ratios.outerBullInner + ratios.outerBullOuter) / 2);
			return { x: 0, y: -r };
		}

		if (target.ring !== "D" || !Number.isFinite(target.value)) {
			return null;
		}
		...
		const midDeg = (angles.start + angles.end) / 2;
		const ringR = radius * ((ratios.doubleInner + ratios.doubleOuter) / 2);
		const point = window.autodartsAnimationShared?.polar
			? window.autodartsAnimationShared.polar(ringR, midDeg)
			: null;
	...
	function emitMoRipple(board, target) {
		if (!mojsLib || typeof mojsLib.Burst !== "function") {
			return;
		}
		const localPoint = getTargetCenterPoint(board.radius, target);
		...
		try {
			const burst = new mojsLib.Burst({
				...
				children: {
					shape: ["circle", "line"],
					stroke: [CONFIG.strokeColor, "rgba(255,255,255,0.95)"],
					strokeWidth: { 5: 0 },
					radius: { 8: 0 },
					duration: 520,
					easing: "quad.out",
			...
			let ring = null;
			if (typeof mojsLib.Shape === "function") {
				ring = new mojsLib.Shape({
					shape: "circle",
					left: screenPoint.x,
					top: screenPoint.y,
					fill: "none",
			...
			activeMoObjects.add(burst);
			burst.replay();
			if (ring && typeof ring.replay === "function") {
				activeMoObjects.add(ring);
				ring.replay();
			}

			setManagedTimeout(() => {
				activeMoObjects.delete(burst);
				if (ring) {
					activeMoObjects.delete(ring);
				}
			}, 750);
		} catch (_) {
			// fail-soft: skip ripple
		}
	}

	function emitRipplesForTargetChange(board, selected, previousToken) {
		if (!Array.isArray(selected) || !selected.length) {
			return;
		}
		const previousSet = new Set(String(previousToken || "").split("|").filter(Boolean));
		selected.forEach((target) => {
			const token = toTargetToken(target);
			if (!token || previousSet.has(token)) {
				return;
			}
			if (target.ring === "D" || target.ring === "DB" || target.ring === "SB") {
				emitMoRipple(board, target);
			}
		});
	}
   ...
   */
	function updateTargets() {
		...
		const selectionToken = selected.map((target) => toTargetToken(target)).join("|");
		const previousToken = lastSelectionToken;
		lastSelectionToken = selectionToken;
		const board = findBoard();
		if (! board) {
			return;
		}
		...
		if (selectionToken !== previousToken) {
			emitRipplesForTargetChange(board, selected, previousToken);
		}
   ...
   */
	...
	debugLog("init", { debug: DEBUG_ENABLED, usesMojs: Boolean(mojsLib && typeof mojsLib.Burst === "function") });
})();
